...
object BenchmarkRequester {
   ...
   */
  case class RequesterStats(id: String, interval: Interval, invokesSent: Int, updatesRcvd: Int) {
    private val durationMs = interval.toDurationMillis

    val invokeSentPerSec = if (durationMs == 0) 0.0 else invokesSent * 1000.0 / durationMs
    val updateRcvdPerSec = if (durationMs == 0) 0.0 else updatesRcvd * 1000.0 / durationMs
  }
}